		...
		<div class="ui_content">
			<table cellspacing="0" cellpadding="0" width="100%" align="left" border="0">
				...
				<tr>
					...
					<td class="ui_text_lt">
						<select id="dept" name="dept.id"  class="ui_select01" value="${entity.dept.id}">
							<option value="-1">--请选择--</option>
							<c:forEach items="${depts}" var="d">
								<option value="${d.id}" <c:if test="${entity.dept.id == d.id }">selected="selected"</c:if> >${d.name}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
